...
import { User, MapPin, Phone, Briefcase, LogOut, Save } from 'lucide-react';
  ...
  return (
    <div style={{ paddingBottom: '90px', fontFamily: 'Arial, sans-serif', minHeight: '100vh', background: '#f8fafc' }}>
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={inputGroupStyle}>
          <label style={labelStyle}><Briefcase size={16} /> Skills</label>
          <input 
            value={profile.skills || ''} 
            onChange={(e) => setProfile({...profile, skills: e.target.value})}
            placeholder="e.g. Driver, Plumber"
            style={inputStyle} 
          />
        </div>
        ...
        <button 
          ...
          disabled={saving}
          style={{ padding: '15px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '12px', fontSize: '1rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginTop: '10px' }}
        >
